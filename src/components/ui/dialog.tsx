...
function DialogOverlay({
  ...
  return (
    <DialogPrimitive.Overlay
      ...
      className={cn(
        "fixed inset-0 isolate z-50 bg-black/10 duration-100 supports-backdrop-filter:backdrop-blur-xs data-[state=closed]:pointer-events-none data-[state=closed]:opacity-0 data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=closed]:animate-out data-[state=closed]:fade-out-0",
        className
      )}
      {...props}
    />
    ...
    <DialogPortal forceMount={forceMount}>
      ...
      >
        ...
        {showCloseButton && (
          ...
          <Button
            variant="ghost"
className="absolute top-2 right-2 size-7 border-border/25 cursor-pointer bg-foreground/15 hover:bg-border/25 transition-colors duration-300"
          size="icon-sm"
          >
              <XIcon
              />
    ...
    >
      ...
      {showCloseButton && (
        <DialogPrimitive.Close asChild>
          <Button variant="outline" className="h-8 border-border/25 cursor-pointer bg-foreground/15 hover:bg-border/25 transition-colors duration-300">Close</Button>
        </DialogPrimitive.Close>
      )}
    </div>
  )
